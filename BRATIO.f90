...
!
   ...
   10 if (b0 < min ( eps, eps * a0 ) ) go to 80
      ...
      if (a0 .ge. min ( 0.2, REAL(b0) ) ) go to 100
...
!
!          myErfc1(ind,x) = myErfc(x)            if ind = 0
!          myErfc1(ind,x) = exp(x*x)*myErfc(x)   otherwise
!
!  Reference:
!
!    Armido Didonato, Alfred Morris,
...
!
      ...
      myErfc1 = 0.5 + (0.5 - x*(top/bot))
      if (ind /= 0) myErfc1 = exp(t) * myErfc1
      return
!
!  0.5 < abs(x) <= 4
!
...
!
   ...
   30 t = (1.0/x)**2
      ...
      myErfc1 = (c - t*top/bot)/ax
!
!  final assembly
!
   40 if (ind == 0) go to 41
         if (x < 0.0) myErfc1 = 2.0*exp(x*x) - myErfc1
         return
   !41 w = dble(x)*dble(x)
   41 w = x * x
      t = w
      !e = w - dble(t)
      e = w - t  ! rishu
      myErfc1 = ((0.5 + (0.5 - e)) * exp(-t)) * myErfc1
      if (x < 0.0) myErfc1 = 2.0 - myErfc1
      return
!
!  limit value for large negative x
!
   50 myErfc1 = 2.0
      if (ind /= 0) myErfc1 = 2.0*exp(x*x)
      return
!
...
!
  ...
  if ( ax <= 0.5 ) then
    ...
    top = (((r(1)*t + r(2))*t + r(3))*t + r(4))*t + r(5)
    bot = (((s(1)*t + s(2))*t + s(3))*t + s(4))*t + 1.0
    myErf = (c - top/(x2*bot)) / ax
    myErf = 0.5 + (0.5 - exp(-x2)*myErf)
...
subroutine myErf_values ( n_data, x, fx )
USE real_precision
...
!  integer s.
!
!     assume integer s are represented in the n-digit, base-a form
!
...
!
!    Local, real  XMAX1, the smallest positive floating point 
...
!
  ...
  real  aug
  real  den
  !real , parameter :: dx0 = 1.461632144968362341262659542325721325d0
  real , parameter :: dx0 = 1.461632144968362341262659542325721325
  integer  i
  integer  m
  integer  n
  integer  nq
